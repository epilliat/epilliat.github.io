...
I'll begin by introducing the non-parametric isotonic model, since it offers a framework for addressing all three problems: label recovery, worker ranking, and ability estimation.
...
The final section will focus on my contribution - the new method I've developed and some of the key insights that emerge from this work.
...
1. [Non parametric isotonic model]{style="background-color: yellow;"} for estimating
   - binary labels
   - rank of workers
   - their abilities
2. [Minimax and polynomial time]{style="background-color: yellow;"} method ISV to do the above three points (at least when $n=d$) 
3. [Not knowing the labels is not harder]{style="background-color: yellow;"} than knowing them for
   - ranking
   - estimating abilities
