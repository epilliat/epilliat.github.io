---
title: "Linear Model and Inference"
format: 
  revealjs:
    incremental: true
    callout-icon: false
...
We observe $n$ individuals, and variables [$Y \in \mathbb R^n$]{style="background-color: orange;"}  and [$(X^{(1)}, \dots, X^{(p)}) \in \mathbb R^{n \times p}$]{style="background-color: lightblue;"}.

. . .

In other words, we observe 
\
[$Y= (Y_1, \dots, Y_n) \in \mathbb R^n$ ]{style="background-color: orange;"}
\
...
We can take infinitely many possible $\beta$
