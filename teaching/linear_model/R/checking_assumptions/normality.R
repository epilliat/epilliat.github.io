...
shapiro_results <- data.frame(
  ...
  W_Statistic = c(sw_normal$statistic, sw_heavy$statistic, 
                  sw_skewed$statistic, sw_bimodal$statistic),
  p_value = c(sw_normal$p.value, sw_heavy$p.value, 
...
ks_results <- data.frame(
  Model = c("Normal", "Heavy-Tailed", "Skewed", "Bimodal"),
  D_Statistic = c(ks_normal$statistic, ks_heavy$statistic, 
                  ks_skewed$statistic, ks_bimodal$statistic),
  p_value = c(ks_normal$p.value, ks_heavy$p.value, 
...
comparison <- data.frame(
  Model = c("Normal", "Heavy-Tailed", "Skewed", "Bimodal"),
  Shapiro_p = round(c(sw_normal$p.value, sw_heavy$p.value, 
                      sw_skewed$p.value, sw_bimodal$p.value), 4),
  KS_p = round(c(ks_normal$p.value, ks_heavy$p.value, 
                 ks_skewed$p.value, ks_bimodal$p.value), 4),
  Both_Agree = c(
