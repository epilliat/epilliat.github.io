...
For the variance, assume that $\mathbb E[\varepsilon_i] = \mu_4 < +\infty$. We have
...
$$\mathbb{E}[\hat \sigma^4] = \sigma^4[\text{tr}(P_{[X]^\perp})^2 + 2\text{tr}(P_{[X]^\perp}^2)] + (\mu_4 - 3\sigma^4)\text{tr}(P_{[X]^\perp}^2)$$

We have

$P_{[X]^\perp}^2 = P_{[X]^\perp}$, so $\text{tr}(P_{[X]^\perp}^2) = \text{tr}(P_{[X]^\perp}) = n-p$

Therefore:
$$\mathbb{E}[\hat \sigma^4] = \sigma^4[(n-p)^2 + 2(n-p)] + (\mu_4 - 3\sigma^4)(n-p)$$

This gives us:
$$\mathbb{V}(\hat{\sigma}^2) = \frac{1}{(n-p)^2}\mathbb{E}[\hat \sigma^4] - \sigma^4 = \frac{2\sigma^4}{n-p} + \frac{\mu_4 - 3\sigma^4}{(n-p)^2}$$
