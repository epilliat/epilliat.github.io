...
2. Answer the initial question using a chi-squared test at a $0.05$ significance level.
...
1. Formulate the hypothesis testing problem to test whether the number of new clients is homogeneous across the regions.
2. What do $\overline X_k$, $\overline X$, $V_k$, $V_W$, $V_B$, $V_T$ represent?
3. Prove the analysis of variance formula:
$$V_T = V_W + V_B \; .$$
*Subtract and add $\overline X_k$ in the definition of $V_T$.*
...
Let $X=(X_1, \dots, X_N)$ be a Gaussian vector $\mathcal N(0, I_N)$ in $\mathbb R^N$ (i.e. $X_i$ are iid $\mathcal N(0,1)$).

1. a. What is the distribution of $QX$, if $Q$ is an orthogonal matrix? ($QQ^T = I_N$)
   b. What is the distribution of $\|PX\|^2$ if $P$ is an orthogonal projector? \
*Use the rank of $P$ defined as $rk(P) = \dim(\mathrm{Im}(P))$.* \
*Definition of orthogonal projector: $P^2=P$ and $P = P^T$.*
   c. Show that if $P$ is an orthogonal projector, then $PX$ is independent of $(I-P)X$. \
*Use the fact that two centered Gaussian vectors $X$, $Y$ are independent if and only if $\mathbb E[X_iY_j] = 0$ for all $i,j$. Translate this into matrix form.*
   d. What is the distribution of $\dfrac{N - rk(P)}{rk(P)}\dfrac{\|PX\|^2}{\|(I-P)X\|^2}$?
   e. Show that if $P$, $P_0$ are two orthogonal projectors such that $\mathrm{Im}(P_0) \subset \mathrm{Im}(P)$, then $P(I-P_0)X$ is independent of $(I-P)(I-P_0)X$. What is the distribution of $\|P(I-P_0)X\|^2$? \
*Show first that $PP_0 = P_0P = P_0$, and that $P - P_0$ is an orthogonal projector.*
   f. What is the orthogonal projector $P_0$ onto $\mathrm{Span}(1, \dots, 1)$? Deduce that $(X_i - \overline X)$ is independent of $\overline X$ for all $i$.

2. We divide $N$ into $d$ blocks: $N = N_1 + \dots + N_d$. We write $(X_1, \dots, X_N) = ((X_{11}, \dots, X_{N_1 1}), (X_{12}, \dots, X_{N_2 2}), \dots, (X_{1d}, \dots, X_{N_d d}))$.
   a. What is the orthogonal projector onto $\mathrm{Span}(E_k)$, where $E_k$ is the vector equal to $1$ on block $k$ and $0$ elsewhere?
   b. Give the orthogonal projector $P$ onto $\mathrm{Span}(E_1, \dots, E_d)$. Explicit $(I-P)(I-P_0)X$ and $P(I-P_0)X$.
   c. Deduce the distribution of $\dfrac{d-1}{N-d}\dfrac{\|P(I-P_0)X\|^2}{\|(I-P)(I-P_0)X\|^2}$ and of the ANOVA test statistic under $H_0$.