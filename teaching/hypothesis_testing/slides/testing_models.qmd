...
::: {.callout-note title="pvalue: definition"}
We define $\mathrm{pvalue}(x_{\mathrm{obs}}) =\mathbb P(\psi(X) > x_{\mathrm{obs}})$ for a right-tailed test.

For a two-tailed test, $\mathrm{pvalue}(x_{\mathrm{obs}}) =2\min(\mathbb P(\psi(X) > x_{\mathrm{obs}}),\mathbb P(\psi(X) < x_{\mathrm{obs}}))$
:::
::: {.callout-note title="pvalue us uniform under $H_0$"}
Under $H_0$, for a pivotal test statistic $\psi$, $p_{\mathrm{value}}(X_{\mathrm{obs}})$ has a uniform distribution $\mathcal U([0,1])$.
:::
