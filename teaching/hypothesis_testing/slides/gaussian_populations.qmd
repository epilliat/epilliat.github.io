...
::: {.callout-note}
## Proposition: distribution of T under H₀
...
We observe $X=(X_1, \dots, X_{n})$ iid  $\mathcal N(\mu_1, \sigma_1^2)$ and $Y=(Y_1, \dots, Y_{n_2})$ iid  $\mathcal N(\mu_2, \sigma_2^2)$.
We also assume that $X$ and $Y$ are independent
...
::: {.callout-note}
## F-Test Statistic
The F-Test Statistic of the Variances (ANOVA) is
$$
...
Under the distribution given by parameters $\mu_1, \mu_2, \sigma_1, \sigma_2$,
$\psi(X,Y)=\frac{\hat \sigma^2_1}{\hat \sigma_2^2}$ has distribution $\frac{\sigma^2_1}{\sigma_2^2} \mathcal F(n_1-1, n_2-1)$
:::
...
Event if the $X_i$ are not standard Gaussian, we can approximate e.g. [$\sqrt{\tfrac{n_1}{\sigma_1^2}}(\overline X - \mu_1)$ by a $\mathcal N(0,1)$]{style="background-color: lightblue;"} using the CLT.

. . .


[Insight: centered and normalized]{style="background-color: yellow;"} variables always look like gaussians under independency assumption.
