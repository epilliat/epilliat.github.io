...
::: {.callout}
- $d$ independent groups (bags), each containing $N_1, \dots, N_d$ individuals. $N_{\mathrm{tot}} = \sum_{j=1}^d N_j$
- Group $j$: $(X_{1,j}, \dots, X_{N_j,j}) \in \mathbb R^{N_j}$ are iid $\mathcal N(\mu_j, \sigma)$. Ex: $X_{ij} =$ sallary of $i$ living in region $j$
- $H_0: \mu_1 = \dots = \mu_d$ vs $H_1: \mu_i \neq \mu_k$ for some $(i,k)$ (**unknown $\sigma$**)
:::
...
- Empirical mean of group $j$:
$\overline X^{(j)} = \tfrac{1}{N_j}\sum_{k=1}^{N_j} X_{ik}$
- Sum of Squares in group $j$: \
$SS_j = \sum_{i=1}^{N_j} (X_{ij} - \overline X^{(j)})^2 \sim \sigma^2\chi^2(N_j-1)$ (under $H_0$)
- Var of group $j$: $V_j = \tfrac{1}{N_j}SS_j$

:::
::: {.column}

- Total empirical mean:
$\overline X = \tfrac{1}{N_{\mathrm{tot}}}\sum_{j=1}^d\sum_{k=1}^{N_j} X_{ik}$
- Sum of Squares btween Groups: $SSB = \sum_{i=1}^{d} N_j(\overline X^{(j)} - \overline X)^2\sim\sigma^2\chi^2(d-1)$ (under $H_0$)
:::
::: 
:::
...
- $\psi(X) = \frac{\tfrac{1}{N_{\mathrm{tot}} - d}\sum_{j=1}^d SS_j}{\tfrac{1}{d-1}SSB}$
...
$$
\begin{aligned}
\mathbb P(X - Y \leq t) &=\int_{-\infty}^t \mathbb P(X \leq t+y)p(y)dy =\int_{-\infty}^t \mathbb P(Y \leq t+x)p(x)dy = \mathbb P(Y-X \leq t) \; .
\end{aligned}
$$
...
- $H_1:$ The median of  $(X_i - Y_i)$ is not $0$ for all $i$
:::
