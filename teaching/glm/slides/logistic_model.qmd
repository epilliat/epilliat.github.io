...
- [$Y$: 1 if BMI > 35]{style="background-color: yellow;"}, 0 otherwise
- AGE
...
For someone for which WALK1=0 and [ACTIV1=0]{style="background-color: yellow;"}:
...
For someone for which WALK1=0 and [ACTIV1=1]{style="background-color: yellow;"}:
...
If $x_1^{(j)} > x_2^{(j)}$. Since [$OR(x_1^{(j)}, x_2^{(j)}) \geq 1 \Leftrightarrow \beta_j \geq 0$]{style="background-color: lightblue;"} rejection region at level $\alpha$ is
...
**Significance Test**: We compare $D$ to the null deviance $D_0 = 4610.8$: [$D_0 - D = 151.3$]{style="background-color: lightblue;"}. The p-value of the test equals [$1 - \chi^2_{10}(151.3) \approx 0$]{style="background-color: lightblue;"} where $\chi^2_{10}$ is the cdf of a $\chi^2_{10}$.
...
## Example: Significance Test of MARITAL
...
## Example: Significance Test of AGE²
...
The deviance test with the previous model has p-value [$1 - F_1(4462.7 - 4439.5) = 1 - F_1(23.2) \approx 10^{-6}$]{style="background-color: lightblue;"}

. . .

This model is [therefore preferable]{style="background-color: yellow;"}, (confirmed with AIC).

. . .

However, we [cannot compare this model with the first]{style="background-color: orange;"} one by deviance test because they are [not nested]{style="background-color: orange;"}.

. . .

We [can however compare their AIC]{style="background-color: lightgreen;"}: this model is preferable.
...
Suppose we are interested in a [new individual]{style="background-color: yellow;"} for whom
...
We want to [predict $Y$]{style="background-color: yellow;"} for this [new individual]{style="background-color: yellow;"}.

## Recall in the Logit Model

. . .

If we have [fitted a logistic regression]{style="background-color: yellow;"} model, we can estimate
...
1. how to construct a [confidence interval around the estimation $p_{\hat{\beta}}(x)$]{style="background-color: yellow;"};


2. how to exploit this estimation to [classify the new individual]{style="background-color: yellow;"} in category $Y = 0$ or $Y = 1$.

## CI: Asymptotic Distribution of p_β(x)
...
- $X=(X^{(1)}, \dots, X^{(p)})$ is the $n \times p$ [design matrix]{style="background-color: yellow;"}
- $W_{\hat{\beta}}$ is the $n \times n$ [diagonal matrix]{style="background-color: yellow;"} with coefs [$p_{\hat \beta}(x_i)(1-p_{\hat \beta}(x_i))$]{style="background-color: lightblue;"}
...
We deduce that when $n \to +\infty$, [$x^T \hat{\beta} \sim N(x^T \beta, x^T (X^T W_{\hat{\beta}} X)^{-1} x)$ ]{style="background-color: lightblue;"}, and the asymptotic CI of $x^T\beta$:
...
## CI for Probability pβ(x)

. . .

Since [$p_{\hat{\beta}}(x) = \text{logit}^{-1}(x^T \hat{\beta})$]{style="background-color: lightblue;"}, we have therefore by application of the increasing function $\text{logit}^{-1}$, the CI at asymptotic level $1 - \alpha$:
...
For a [threshold to choose $s \in [0, 1]$]{style="background-color: yellow;"}, we use the rule:
...
The "natural" choice of threshold is $s = 0.5$ but [this choice can be optimized]{style="background-color: yellow;"}.

## Evaluation of Classification Quality

. . .

We proceed by [cross-validation]{style="background-color: yellow;"}: 

. . .

Using a [train sample]{style="background-color: yellow;"}, predict $Y$ on a [test sample]{style="background-color: yellow;"} and form the [confusion matrix]{style="background-color: yellow;"}.
...
**FP**: false positives: number of individuals who were classified [positive]{style="background-color: yellow;"} who were actually [negative]{style="background-color: orange;"}


## Evaluation of Classification Quality
. . .

The ideal is to have a confusion matrix that is [as diagonal as possible]{style="background-color: yellow;"}.

. . .

We generally seek to maximize the [following indicators]{style="background-color: yellow;"}:
...
The **sensitivity** (or recall, or true positive rate) estimates [$\P(\hat{Y} = 1|Y = 1)$]{style="background-color: lightblue;"} by 
...
The **specificity** (or selectivity, or true negative rate) estimates [$\P(\hat{Y} = 0|Y = 0)$]{style="background-color: lightblue;"}
...
We finally choose the [optimal threshold $s$]{style="background-color: yellow;"}, according to the [chosen score]{style="background-color: yellow;"}.
...
$\hat Y=1$ (treatment) while the patient is [not ill]{style="background-color: yellow;"} ($Y = 0$)

. . .

$\hat Y=0$ (no treatment) while the patient [has a serious illness]{style="background-color: yellow;"} ($Y=1$)
...
The [AUC (area under the curve)]{style="background-color: yellow;"} is a quality indicator of the model ($0 \leq \text{AUC} \leq 1$). 

. . .

Or equivalently, the [Gini index: $2 \times \text{AUC} - 1$]{style="background-color: yellow;"}.

. . .

**Use**: compare $2$ models by [plotting the 2 ROC curves]{style="background-color: yellow;"}. 
